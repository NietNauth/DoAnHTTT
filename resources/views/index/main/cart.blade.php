        ...
        @if(!empty($cart) && count($cart) > 0)
                    ...
                    <tbody>
                        ...
                        @foreach($cart as $cartKey => $item)
                            ...
                            <tr class="border-b">
                                ...
                                {{-- Số lượng --}}
                                <td class="px-6 py-4 text-center">
                                    <form action="{{ url('cart/update') }}" method="POST" class="flex items-center justify-center gap-2">
                                        @csrf
                                        <input type="hidden" name="cartKey" value="{{ $cartKey }}">
                                        <input type="number" name="soLuong" value="{{ $item['soLuong'] }}" min="1"
                                               class="w-16 px-2 py-1 border rounded text-center">
                                        <button type="submit"
                                                class="bg-[#55d5d2] hover:bg-[#48c2bf] text-white px-3 py-1 rounded transition">
                                            Cập nhật
                                        </button>
                                    </form>
                                </td>
                                ...
                                {{-- Xóa --}}
                                <td class="px-6 py-4 text-center">
                                    <form action="{{ url('cart/remove') }}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xoá sản phẩm này?');">
                                        @csrf
                                        <input type="hidden" name="cartKey" value="{{ $cartKey }}">
            ...
            <div class="mt-6 flex flex-col md:flex-row md:justify-between md:items-center">
                ...
                <div class="mt-4 md:mt-0 flex gap-4">
                    <form action="{{ url('cart/clear') }}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xoá toàn bộ giỏ hàng?');">
                        @csrf
